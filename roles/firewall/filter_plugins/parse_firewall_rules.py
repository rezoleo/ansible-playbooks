...
def parse_firewall_rule(rule: dict[str, Any]) -> str:
        ...
        if rule["protocol"].lower() in allowed_protocols:
            ...
            if "destination_port" in rule:
                if rule["protocol"].lower() in ["tcp", "udp"]:
                    builder.extend(
                        ["--destination-port", str(rule["destination_port"])]
                    )
                    rule.pop("destination_port")
                else:
                    raise AnsibleFilterError(
                        f"The rule \"{rule_name}\" should not contain a 'destination_port' key. 'destination_port' should be used with tcp or udp!"
                    )
            rule.pop("protocol")
        else:
            raise AnsibleFilterError(
                f'The protocol "{rule["protocol"]}" in rule "{rule_name}" is not a valid protocol! Allowed protocols are {", ".join(allowed_protocols)}.'
            )
    ...
    if "raw_extras" in rule:
        if isinstance(rule["raw_extras"], list):
            ...
            raise AnsibleFilterError(
                f'The extras in rule "{rule_name}" should be a list or a string!'
            )
        rule.pop("raw_extras")

    if len(rule) != 0:
        raise AnsibleFilterError(
            f'Unrecognized {"property" if len(rule) == 1 else "properties"} in rule "{rule_name}": {", ".join(list(rule.keys()))}.'
        )
...
def validate_firewall_rules(rule_set: dict[str, list]) -> dict[str, list]:
    ...
    for name, rules in rule_set.items():
        ...
        for rule in rules:
            if not isinstance(rule, dict):
                raise AnsibleFilterError(
                    f'The rule "{rule}" in {name} rule set should be a dictionary!'
                )
    return rule_set
